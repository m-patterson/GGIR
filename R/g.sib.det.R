g.sib.det = function(M,IMP,I,twd=c(-12,12),anglethreshold = 5,
  ...
  sptwindow_HDCZA = function(start_time, block, idx_start, idx_end, angle, k =60, perc = 0.1, inbedthreshold = 15,
  ...
  if (ND > 0.2) {
    ...
    if (countmidn != 0) {
      if (countmidn == 1) {
        ...
        inbedout = sptwindow_HDCZA(start_time = tmpTIME[qqq1], block = '1 midnight',idx_start=qqq1,idx_end=qqq2,tmpANGLE,ws3=ws3,constrain2range=constrain2range,
        ...
        if (length(inbedout$sptwindow_HDCZA_end) != 0 & length(inbedout$sptwindow_HDCZA_start) != 0) {
          if (inbedout$sptwindow_HDCZA_end+qqq1 >= qqq2-(1*(3600/ws3))) {
            ...
            if (newqqq1 < length(angle) & (newqqq2 - newqqq1) > (23*(3600/ws3)) ) {
              inbedout = sptwindow_HDCZA(start_time = tmpTIME[newqqq1], block = '1 midnight re-run ',idx_start=newqqq1,idx_end=newqqq2,angle[newqqq1:newqqq2],ws3=ws3,constrain2range=constrain2range,
                                         perc = perc, inbedthreshold = inbedthreshold, bedblocksize = bedblocksize,
                                         outofbedsize = outofbedsize)
        ...
        for (j in 1:(countmidn)) { #-1
          ...
          if (length(inbedout$sptwindow_HDCZA_end) != 0 & length(inbedout$sptwindow_HDCZA_start) != 0) {
            if (inbedout$sptwindow_HDCZA_end+qqq1 >= qqq2-(1*(3600/ws3))) {
              ...
              # only try to extract SPT again if it is possible to extrat a window of more than there is more than 23 hour
              if (newqqq1 < length(angle) & (newqqq2 - newqqq1) > (23*(3600/ws3)) ) {
                inbedout = sptwindow_HDCZA(start_time = tmpTIME[newqqq1], block='6pm to 6pm',idx_start=newqqq1,idx_end=newqqq2,angle[newqqq1:newqqq2],ws3=ws3,constrain2range=constrain2range,
                                           perc = perc, inbedthreshold = inbedthreshold, bedblocksize = bedblocksize,
                                           outofbedsize = outofbedsize)
                if (inbedout$sptwindow_HDCZA_start+newqqq1 >= newqqq2) {
                  inbedout$sptwindow_HDCZA_start = (newqqq2-newqqq1)-1
